...
global {
	...
	file shape_file_ndvi <- file("../includes/ndvi_zone_etude.shp");
	geometry shape <- envelope(shape_file_appetence);

	//---------------------------------------les paramètres----------------------------------------
	date starting_date <- date([2020, 10, 15, 7, 0]);
	float step <- 1 #days;
	int nb_trp <- 40 min: 2 parameter: 'Nb_trp';
	int cpt_trp_aller;
	int cpt_trp_retour;
	...
	//------------------------- diffusion --------
	init {
	//create s_ndvi from: shape_file_ndvi with: [ndvi::int(get("vndvi"))];
...
grid espace cell_width: largeur_cellule cell_height: hauteur_cellule neighbors: 8 {
	...
	init {
		...
		if self.e_pasto != "N" or self.e_pasto = "P1" or self.e_pasto = "P2" or self.e_pasto = "P3" or self.e_pasto = "P4" {
			...
			r <- r_init;
			//regulièrement mise à jour dans le reflex de mise à jour des couleurs
		}
		//---------------------- qualité du fourrage ----------------------
		/*if self.e_pasto = nil {
			self.e_pasto <- one_of(["N", "P1", "P2", "P3", "P4"]);
			//write e_pasto;
		}*/
		if e_pasto = "P1" or "P2" {
			beta3 <- -100;
	...
	//------------- initialisation de la vegetation dans la grille en tenant compte du ndvi --------
	reflex veg_nvdi when: current_date <= date([2020, 10, 17]) and test_nvdi {
		ask s_ndvi overlapping (self) {
			myself.val_nvdi <- myself.val_nvdi + self.ndvi_norm;
			myself.cpt_nvdi <- myself.cpt_nvdi + 1;
			//write myself.cpt_nvdi;
		}

		r <- r * (val_nvdi / cpt_nvdi);
		//write "1";
		test_nvdi <- false;
	}

	//------------------------------------------------------------------------
	reflex vegetation_microfaune when: self.e_pasto != "N" and self.color != #grey {
...
species troupeau skills: [moving] {
	...
	reflex creation_rs_1 when: i > 0 and i < 2 {
		...
		create res_social number: 1 {
			if one_of(espace where (each.e_pasto != #red and (each.location.y >= terr_orig.location.y + 10 #km and each.location.y < terr_orig.location.y + 20 #km) and (terr_orig.x - 50
	...
	reflex chez_res_soc when: location distance_to ((members closest_to (self)).location) <= largeur_cellule and en_zone_acc = false and location != terr_acc and
	objectif = 'deplacement' {
	...
	reflex manque_bio_rs when: signal and en_zone_acc and fin_transhumance = false and objectif = 'recherche_bio' {
		if rs_manq_bio {
			create res_social number: rnd(5) {
...
experiment mecanism type: gui {
	output {
		...
		display graphique type: java2D refresh: every(1 #week) {
			...
			chart "Impact of the herbivores on the vegetation" type: series x_label: 'day' y_label: "herds consommation proportion" size: {0.5, 0.5} position: {0.5, 0} {
			...
			chart "Evolution of vegetation" type: series x_label: 'day' y_label: "dry matter quantity (Kg/Ha)" size: {0.5, 0.5} position: {0, 0.5} {
				data "Evolution of vegetation" value: evolution_veg color: #green;
			}
