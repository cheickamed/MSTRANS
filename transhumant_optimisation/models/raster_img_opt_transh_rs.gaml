...
global {
	...
	file shape_file_marche <- file("../includes/parc_marche_sen_coupe.shp");
	geometry shape <- envelope(shape_file_appetence);

	//---------------------------------------les paramètres----------------------------------------
	date starting_date <- date([2020, 10, 15, 7, 0]);
	float step <- 1 #days;
	int nb_trp <- 7 min: 2 parameter: 'Nb_trp';
	int nb_bovin <- 0;
	int np_pti_rum <- 0;
	int cpt_trp_aller;
	int cpt_trp_retour;
	int moy_cycle_aller_bovin <- 0;
	int moy_cycle_retour_bovin <- 0;
	int moy_cycle_aller_peti_rum <- 0;
	int moy_cycle_retour_peti_rum <- 0;
	/*float std_cycle_aller_micro_bovin <- 0.0;
	float std_cycle_retour_micro_bovin <- 0.0;
	float std_cycle_aller_micro_peti_rum <- 0.0;
	float std_cycle_retour_micro_peti_rum <- 0.0;*/
	...
	//Réseau social
	bool rs_exist <- false parameter: 'Social' category: 'Reseau_social';
	...
	string plvt;
	//interaction trp_veg
	float d_rech <- 30 #km min: 15 #km parameter: "dist-recherche-bio" category: "Impact troupeau-végétation";
	...
	float vitesse_aller <- 19.6 min: 15.0 max: 20.0 parameter: 'Vitesse aller' category: "Mecanisme optimisation";
	float vitesse_retour <- 33.0 min: vitesse_aller max: 37.0 parameter: 'Vitesse retour' category: "Mecanisme optimisation";
	...
	init {
		...
		//--------------- image raster ------------
		//map_init <- file(map_init as_matrix {87, 82}); //avec le redimentionnement plus besoin de ette ligne
		ask espace where (each.grid_x >= 0 and each.grid_y >= 0) {
	...
	/*reflex update_veg_color when: every(step) and is_batch = true {
		ask espace {
			if e_pasto = "N" {
				self.color <- #red;
				self.r <- 1.0;
...
//----------------- debut operation de diffusion -----------
	...
	reflex valeurs_influences when: diff_eau = false and cycle < 2 {
		...
		diffuse var: influence_infras_veto on: espace where (each.esp_infrast_veto = true) propagation: gradient; // radius: 2
			...
			if self.fin_transhumance = false {
				ask espace overlapping (self.location) {
					if s_pluie != 0 { // retour en fonction du FIT
						...
						myself.speed <- gauss(vitesse_retour, 2);
	...
	reflex trp_dure_orig_acc when: nb_trp - 2 <= cpt_trp_aller and cpt_trp_aller < nb_trp {
		nb_cycle_aller <- round((current_date - starting_date) / 86400); // Dans Gama les date sont en seconde
		//durée approximative de la phase aller de la transhumance
		moy_cycle_aller_bovin <- round(mean(troupeau collect (each.cycle_aller_micro_bovin)));
		moy_cycle_aller_peti_rum <- round(mean(troupeau collect (each.cycle_aller_micro_peti_rum)));
		//std_cycle_aller_micro_bovin <- standard_deviation(troupeau collect (each.cycle_aller_micro_bovin));
		//std_cycle_aller_micro_peti_rum <- standard_deviation(troupeau collect (each.cycle_aller_micro_peti_rum));
		//do pause;
	}

	reflex trp_dure_acc_orig when: nb_trp - 2 <= cpt_trp_retour and cpt_trp_retour <= nb_trp - 1 {
		nb_cycle_retour <- round((current_date - date([2021, 6, 15])) / 86400); // Dans Gama les date sont en seconde
		//durée approximative de la phase aller de la transhumance
		moy_cycle_retour_bovin <- round(mean(troupeau collect (each.cycle_retour_micro_bovin)));
		moy_cycle_retour_peti_rum <- round(mean(troupeau collect (each.cycle_retour_micro_peti_rum)));
		//std_cycle_retour_micro_bovin <- standard_deviation(troupeau collect (each.cycle_retour_micro_bovin));
		//std_cycle_retour_micro_peti_rum <- standard_deviation(troupeau collect (each.cycle_retour_micro_peti_rum));
		//do pause;
	}

	//----------------------- sauvegarde indice macros
	reflex save_dveg when: is_batch and cpt_trp_retour >= nb_trp {
		save [nb_cycle_aller, moy_cycle_aller_bovin, moy_cycle_aller_peti_rum, propor_utilisa_aller] rewrite: false to: "dist_vegetation_espace_entier_aller.csv" type: csv;
		save [nb_cycle_retour, moy_cycle_retour_bovin, moy_cycle_retour_peti_rum, propor_utilisa_retour] rewrite: false to: "dist_vegetation_espace_entier_retour.csv" type: csv;
	}
	//-------------------------------------------
} // fin du global
...
grid espace cell_width: largeur_cellule cell_height: hauteur_cellule neighbors: 8 {
	...
	float r_init;
	// <- 4.8 * 10 ^ 8; //Afin de simuler le debut de la saison des pluies ou la vegetation est plus
	float seuil_r;
	int surpature <- 0;
	rgb color2;
...
//-------------------------- processus d'optimisation -----------------------
	...
	//-----------------------------------------------------------------------
	reflex fit when: current_date >= d_cetcelde and current_date <= fin_sai_pluie {
...
species troupeau skills: [moving] {
	...
	init {
		...
		if rs_exist { //creation du réseau social en terroir d'accueil
			create res_social number: rnd(1, elmt_res_social) {
				location <- one_of(espace where (each.color != #red and (terr_acc.location.y - 30 #km < each.location.x and each.location.y >= terr_acc.location.y + 30 #km) and
				(terr_orig.x - d_res_soc < each.location.x and each.location.x < terr_orig.x + d_res_soc) and (each.influence_eau != 0 or each.influence_infras_marche != 0))).location;
			}
	...
	reflex creation_rs_1 when: creation_rs = false and diffusion1 = true { //i > 1 and i < 4 and 
		...
		//creation des elements de rs présents sur le chemin de transhumance
		// on ne cré pas de réseau social dans des cellules non pâturables
		if init_grille = true and rs_exist {
			create res_social number: rnd(elmt_res_social, res_soc_za and rs_exist ? 9 : 10) {
				self.location <- one_of(espace where (each.color != #red and (each.location.y >= terr_orig.location.y + 10 #km) and (terr_orig.x - d_res_soc < each.location.x and
				each.location.x < terr_orig.x + d_res_soc) and (each.influence_eau != 0 or each.influence_infras_marche != 0))).location;
			}

		}

		// création du réseau social de chaque transhumant en zone d'accueil
		if res_soc_za and rs_exist {
			create res_social {
				location <- terr_acc;
			}
	...
	reflex chez_veterinaire when: my_cell.esp_infrast_veto and fin_transhumance = false and soin = false { //location distance_to (one_of(espace where (each.esp_infrast_veto)))
		the_target <- nil;
		k1 <- k1 + 1;
		soin <- true;
		//write 'k1';
	...
	reflex chez_res_soc when: en_zone_acc = false and location != terr_acc and objectif = 'deplacement' and diffusion1 = true and rs_exist {
	//chez le réseau social différent de celui que le transhumant a éventuellement en zone d'accueil
		if members != [] {
			if self distance_to (members closest_to (self)) <= largeur_cellule {
				the_target <- nil;
				k2 <- k2 + 1;
	...
	reflex quiter_res_social when: objectif = 'quitter_res_soc' and location != terr_acc and en_zone_acc = false and rs_exist {
	...
	bool relocalisation <- true;
	point the_target1;

	reflex aller_za when: fin_transhumance = false and current_date >= date_dep and presence_ter_acc = 0 {
		do goto target: the_target;
		...
		if location = terr_acc { //and objectif != 'recherche_bio'
			if rs_exist and the_target != nil {
				if res_soc_za and relocalisation { //existence de rs et rs dans son terroir d'accueil
					the_target1 <-
					any_location_in(polygon([{109976.39057523105, 122126.85505475942, 0.0}, {80349.39286259573, 142460.22999907122, 0.0}, {71403.05552267504, 175017.40473455004, 0.0}, {179194.4573483566, 178019.11325810337, 0.0}, {164280.68537360977, 141411.86862750398, 0.0}, {109976.39057523105, 122126.85505475942, 0.0}])).location;
					the_target <- the_target1;
					relocalisation <- false;
				} else {
				// le trp na pas a changer de terroir d'accueil
					the_target <- nil;
				}

				if relocalisation = false and res_soc_za = true and location = the_target1 { //le trp est chez son rs du terroir d'accueil
					the_target <- nil;
				}

			} else { // pas d'existence de réseau social
				the_target <- nil;
			}
			...
			if the_target = nil {
				...
				if bool_cycle_aller {
					if self.type_trp = 'gros_rum' {
						cycle_aller_micro_bovin <- round((current_date - date_dep) / 86400);
					} else {
						cycle_aller_micro_peti_rum <- round((current_date - date_dep) / 86400);
					}
	...
	reflex retour_co when: fin_transhumance and presence_terr_orig = 0 {
		...
		if self.location = terr_orig and arrive_orig {
			...
			the_target <- nil;
			if self.type_trp = 'gros_rum' {
				cycle_retour_micro_bovin <- round((current_date - date_retour) / 86400);
			} else {
				cycle_retour_micro_peti_rum <- round((current_date - date_retour) / 86400);
			}
	...
	//---------------------------- fonction d'optimisation du choix du trajet --------------
		...
		//La phase retour de la transhumance
			ask my_cell neighbors_at (speed * step / largeur_cellule) where (each.location.y < self.location.y and each.location.y >= self.terr_orig.y) { //and each.location.y >= self.location.y - speed * step
			...
			if L1 != [] and self.location distance_to
			(polygon([{64794.08188692952, 31477.025629178388, 0.0}, {69702.86097971332, 33523.56217208016, 0.0}, {197571.02203684137, 64966.325043299, 0.0}, {249901.88101234683, 92196.61045416933, 0.0}, {301463.11581025284, 129504.565354791, 0.0}, {321880.7002303945, 94700.7261662092, 0.0}, {221963.3998252987, 22100.137802265817, 0.0}, {63909.11949671188, -10263.28950715647, 0.0}, {64794.08188692952, 31477.025629178388, 0.0}]).location) >= 40
			#km {
				ind <- flip(0.5) ? L1 index_of (min(L1)) : L1 last_index_of (min(L1));
		...
		//La phase aller de la transhumance
			ask my_cell neighbors_at (speed * step / largeur_cellule) where (each.location.y > self.location.y and each.location.y <= self.terr_acc.y) {
			//and each.location.y <= self.location.y + speed * step
			//le calcul de f se fait en fonction du troupeau le plus proche de la cellule
				if self.r - myself.cons_jour - myself.alpha5 <= 0 {
					myself.L1 <-
					myself.L1 + [self.alpha1 + self.alpha2 * self.cout_eau_cell + self.beta3 * myself.alpha3 + myself.beta4 * self.alpha4 * self.cou_soin + alpha5 * self.cou_completion - myself.a * self.cou_vente + self.cou_soin + self.cou_vol];
	...
	rgb couleur_chemin <- one_of([#red, #green, #black, #magenta, #maroon, #purple, #darkgoldenrod, #saddlebrown, #chocolate, #orange, #gold]);
...
grid grille width: 36 height: 38 neighbors: 8 {
	...
	reflex densite_aller when: cpt_trp_aller >= nb_trp and enregistrement and is_batch {
		save [grid_x, grid_y, s_nb_trp_inside] to: 'forage_fort_ras_opt_aller_75r.csv' rewrite: false type: 'csv';
		s_nb_trp_inside <- 0;
		enregistrement <- false;
	}

	reflex densite_za when: cpt_trp_aller >= nb_trp and enregistrement1 and current_date > date([2021, 6, 15]) and is_batch {
		save [grid_x, grid_y, s_nb_trp_inside] to: 'forage_fort_ras_opt_za_75r.csv' rewrite: false type: 'csv';
		s_nb_trp_inside <- 0;
		enregistrement1 <- false;
	}

	reflex densite_retour_et_surpaturage_localise when: cpt_trp_retour >= nb_trp - 1 and enregistrement2 and is_batch {
		save [grid_x, grid_y, s_nb_trp_inside] to: 'forage_fort_ras_opt_retour_75r.csv' rewrite: false type: 'csv';
		//save [grid_x, grid_y, surpature] to: 'forage_fort_ras_opt_surpaturage_0.33_75r.csv' rewrite: false type: 'csv';
...
experiment multiple_rainfall type: gui {
	...
	//create mecaoptimisation_image_model with: [qt_pluie::550];
		create mecaoptimisation_image_model with: [qt_pluie::375, q_seuil_r::0.33, impt_trp_veg_color::#orange, veg_color::#orange];
		create mecaoptimisation_image_model with: [qt_pluie::150, q_seuil_r::0.33, impt_trp_veg_color::#red, veg_color::#red];
	}

	permanent {
		display courbe_cumul type: java2D toolbar: #blue axes: false {
			chart "(b) Dynamic of herbivore population" type: series position: {0.5, 0} x_label: 'day' y_label: "herds size" size: {0.5, 0.5} {
			...
			chart " (a) Evolution of vegetation" type: series x_label: 'day' y_label: "dry matter quantity (Kg/Ha)" position: {0, 0} size: {0.5, 1} {
			...
			chart "(c) Vegetation gathered by herds" type: series position: {0.5, 0.5} x_label: 'day' y_label: "herds consommation (%)" size: {0.5, 0.5} {
				loop s over: simulations {
...
experiment trace_chemin type: gui {
	output {
		...
		/*display forage_trp type: java2D {
		//grid espace triangulation: true; // lines: #lightgrey;
			species zone aspect: asp_zone transparency: 0.3 refresh: false;
			species forage aspect: asp_forage refresh: false;
			species troupeau aspect: chemin_trp;
...
/*display graphique type: java2D refresh: every(1 #week) {
			chart "(a) Dynamic of herbivore population" type: series x_label: 'day' y_label: "size" size: {0.5, 0.5} position: {0, 0} {
				datalist ["Beef", "Sheep", "Goat"] value: [sum(troupeau collect (each.eff_bovin)), sum(troupeau collect (each.eff_ovin)), sum(troupeau collect (each.eff_caprin))] marker:
				false style: line thickness: 2 color: [#blue, #black, #maroon];
			}
			...
			chart "(d) Proportion of overgrazed cells" type: series x_label: 'day' y_label: "overgrazed cells (%)" size: {0.5, 0.5} position: {0.5, 0.5} {
				data "proportion" value: sous_seuil_veg marker: false style: line thickness: 4 color: #red;
			}
...
//*************************************************************************************************
experiment sauv_donne type: batch repeat: 75 until: cpt_trp_retour >= nb_trp or cycle >= 400 {
	parameter 'Batch mode' var: is_batch <- true;
	parameter 'Social_network' var: rs_exist;
	parameter 'Dist_soc' var: d_res_soc;
	...
	reflex save {
		...
		(each.moy_cycle_aller_bovin)) / length(simulations), standard_deviation(simulations collect (each.moy_cycle_aller_bovin)), sum(simulations collect
		(each.moy_cycle_aller_peti_rum)) / length(simulations), standard_deviation(simulations collect (each.moy_cycle_aller_peti_rum)), sum(simulations collect
		(each.propor_utilisa_aller)) / length(simulations), standard_deviation(simulations collect (each.propor_utilisa_aller))] to: "cycle_forage_fort_ras_opt_aller_75r.txt" type:
		text;
		...
		(each.moy_cycle_retour_bovin)) / length(simulations), standard_deviation(simulations collect (each.moy_cycle_retour_bovin)), sum(simulations collect
		(each.moy_cycle_retour_peti_rum)) / length(simulations), standard_deviation(simulations collect (each.moy_cycle_retour_peti_rum)), sum(simulations collect
		(each.propor_utilisa_retour)) / length(simulations), standard_deviation(simulations collect (each.propor_utilisa_retour))] to: "cycle_forage_fort_ras_opt_retour_75r.txt" type:
		text;
	}
